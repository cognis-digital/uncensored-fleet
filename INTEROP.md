...
How **uncensored-fleet** fits the wider Cognis suite — a composable set of defensive, analytical
tools that all run on your own hardware and speak plain **JSON** (or an OpenAI-compatible
**`/v1`**). `uncensored-fleet` lives in the **Agents + cognition** cluster.
...
| from | relation | to |
|---|---|---|
| `uncensored-fleet` | JSON findings compose with | [`agentlex`](https://github.com/cognis-digital/agentlex), [`humind`](https://github.com/cognis-digital/humind), [`agentsmith`](https://github.com/cognis-digital/agentsmith), [`agentmap`](https://github.com/cognis-digital/agentmap), [`engram`](https://github.com/cognis-digital/engram) |
| `uncensored-fleet` | AI add-ins are served `/v1` by | [`edgemesh`](https://github.com/cognis-digital/edgemesh) (your fleet) |
| `uncensored-fleet` | findings can be narrated / reasoned over by | [`humind`](https://github.com/cognis-digital/humind) -> [`agentlex`](https://github.com/cognis-digital/agentlex) |
| `uncensored-fleet` | export to intel formats via | [`stixgen`](https://github.com/cognis-digital/stixgen) (STIX) / [`attackmap`](https://github.com/cognis-digital/attackmap) (ATT&CK) |

## Composition patterns

Everything reads/writes JSON, so tools chain with ordinary pipes; nothing leaves the box.

**1 — chain within the agents + cognition cluster.** `uncensored-fleet` output feeds the next tool:
```bash
uncensored-fleet ... --format json > out.json          # this tool's findings (see `uncensored-fleet --help`)
...
uncensored-fleet ...                                         # vision / reasoning add-ins light up
```

**3 — export findings to your SOC's formats.**
```bash
uncensored-fleet ... --format json | stixgen from-json > bundle.stix.json   # STIX 2.1
uncensored-fleet ... --format json | attackmap map > attack.json            # ATT&CK techniques
```

**4 — narrate through cognition + agents.** `humind` extracts salience from `uncensored-fleet`'s
output; `agentlex` holds it as KB facts and fires Horn rules to escalate. `uncensored-fleet` slots
